    ...
    [...groups.values()].forEach((items, groupIndex) => {
      items.forEach((file, i) => {
        ...
        result.push({
          id: file,
          position: {
            x: (i % 5) * 230 + (groupIndex % 2) * 40,
            y: y + Math.floor(i / 5) * 100,
          },
          data: {
            label: (
              <div className="cf-node-label">
                <strong>{shortName(file)}</strong>
                <span>{directoryName(file)}</span>
              </div>
            ),
          },
          style: {
  ...
  return (
    <div className="graph-shell">
      <div className="graph-toolbar">
        <Network size={16} />
        <strong>{allFiles.length} FILES · {dependencies.length} IMPORT LINKS</strong>
      ...
      <div className="dependency-canvas">
        <ReactFlow
          ...
          minZoom={0.04}
          maxZoom={2.5}
          onNodeClick={(_, n) => setSelectedFile(n.id)}
        ...
        >
          ...
          <MiniMap
            pannable
            zoomable
            nodeColor={(n) => n.id === selectedFile ? "#61c7ff" : "#24465b"}
            maskColor="rgba(3,8,13,.82)"
            style={{ background: "#071019", border: "1px solid #20394a" }}
          />
        </ReactFlow>
      </div>
    </div>
