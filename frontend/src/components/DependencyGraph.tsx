...
export default function DependencyGraph({ dependencies, files = [] }: Props) {
  ...
  const nodes: Node[] = useMemo(() => {
    ...
    allFiles.forEach((file) => {
      const key = directoryName(file).split("/").slice(0, 3).join("/") || "root";
      groups.set(key, [...(groups.get(key) || []), file]);
    });

    let y = 0;
    const result: Node[] = [];
    [...groups.entries()].forEach(([group, items], groupIndex) => {
      items.sort();
      items.forEach((file, i) => {
        ...
        result.push({
          id: file,
          position: { x: (i % 5) * 230 + (groupIndex % 2) * 45, y: y + Math.floor(i / 5) * 105 },
          data: { label: <div className="cf-node-label"><strong>{shortName(file)}</strong><span>{group}</span></div> },
          style: {
  ...
  return <div className="graph-shell">
    <div className="graph-toolbar">
      <Network size={16} />
      <strong>{allFiles.length} FILES · {realDependencies.length} REAL IMPORT LINKS</strong>
      <div className="graph-search">
        <Search size={13} />
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Find any file in repository..." />
    ...
    <div className="dependency-canvas">
      <ReactFlow nodes={nodes} edges={edges} fitView fitViewOptions={{ padding: 0.18 }} minZoom={0.03} maxZoom={2.5} onNodeClick={(_, n) => setSelectedFile(n.id)} onPaneClick={() => setSelectedFile(null)} nodesDraggable panOnDrag zoomOnScroll>
        <Background gap={24} size={1} color="#183246" />
        <Controls />
        <MiniMap pannable zoomable nodeColor={(n) => n.id === selectedFile ? "#61c7ff" : "#24465b"} maskColor="rgba(3,8,13,.82)" style={{ background: "#071019", border: "1px solid #20394a" }} />
      </ReactFlow>
    </div>
  </div>;
}
